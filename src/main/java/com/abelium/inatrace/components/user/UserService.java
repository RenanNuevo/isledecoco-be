...
public class UserService extends BaseService {
    ...
    @Value("${INATrace.loginManager.mail:registrations@inatrace.com}")
    private String loginManagerMail;
    
    @Value("${INATrace.emailConfirmation.url:http://localhost:4200/confirm-email/}")
    private String emailConfirmationUrl;
    
    @Value("${INATrace.:http://localhost:4200/reset-password/}")
    private String passwordResetUrl;

	@Value("${INATrace.env:dev}")
	private String env;
	...
	public void createUser(ApiCreateUserRequest createUserRequest) throws ApiException {
		...
		if (!"DEMO".equals(env)) {
			...
			logger.error("Can not get location for {}", error);
